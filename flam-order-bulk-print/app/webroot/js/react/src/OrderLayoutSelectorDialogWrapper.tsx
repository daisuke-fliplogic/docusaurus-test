/**
 * 受注一覧 レイアウト選択ダイアログ ラッパー
 * app/webroot/js/react/src/OrderLayoutSelectorDialogWrapper.tsx
 *
 * SaleLayoutSelectorDialogWrapper と同パターン。reportGroupCd="order" を渡す。
 * jQuery 側の bulkPrint() から window.orderLayoutSelectorDialog.open() で呼び出される。
...
class OrderLayoutSelectorDialogWrapper {
  ...
  constructor() {
    ...
    this.mountNode.id = "order-layout-selector-dialog-root";
  ...
  public open(reportGroupCd: string, onSelect: SelectCallback): void {
    this.state = { open: true, reportGroupCd: reportGroupCd || "order", onSelect };
    this.render();
  }
...
declare global {
  interface Window {
    orderLayoutSelectorDialog: OrderLayoutSelectorDialogWrapper;
  }
}

window.orderLayoutSelectorDialog = new OrderLayoutSelectorDialogWrapper();

export default OrderLayoutSelectorDialogWrapper;
